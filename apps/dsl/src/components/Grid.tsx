...
type ColsInput = string | number | ColsRule[] | boolean | null | undefined;

export type GridProps = GridDomProps &
  UtilityPropBag &
...
const Grid = forwardRef<HTMLDivElement, GridProps>(({ children, className, cols = '1-3', ...props }, ref) => {
  const { utilityClassName, rest } = resolveUtilityClassName(props);
  const colsClass =
    typeof cols === 'string' ? gridVariants({ cols: cols as ColsRuleValue }) : resolveColsClassName(cols);

  return (
    <div ref={ref} data-class="Grid" className={cn('grid', colsClass, utilityClassName, className)} {...rest}>
      {children}
    </div>
  );
});
...
type GridColProps = Omit<React.HTMLAttributes<HTMLDivElement>, UtilityPropPrefix> &
  UtilityPropBag & {
    children?: ReactNode;
    span?: number;
    start?: number;
...
const GridCol = forwardRef<HTMLDivElement, GridColProps>(
  ({ children, className, span, start, end, order, ...props }, ref) => {
    ...
    return (
      <div ref={ref} data-class="Grid.Col" className={cn(colClasses, utilityClassName, className)} {...rest}>
        {children}
      </div>
    );
  },
