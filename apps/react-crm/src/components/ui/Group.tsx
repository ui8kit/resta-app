...
export type GroupProps = GroupDomProps &
  UtilityPropBag & {
    children: ReactNode;
...
export const Group = forwardRef<HTMLElement, GroupProps>(
  ({ children, className, component = 'div', grow = false, ...props }, ref) => {
    ...
    return (
      <Element
        ref={ref}
        data-class="group"
        className={cn(defaultProps, specificUtilities, utilityClassName, className)}
        {...rest}
      >
        {children}
      </Element>
