...
function createMockStage(
  ...
  return {
    ...
    enabled,
    dependencies,
    canExecute: typeof canExecute === 'function' ? canExecute : vi.fn().mockReturnValue(canExecute),
...
describe('Pipeline', () => {
  ...
  describe('executeStage', () => {
    ...
    it('should throw when stage not found', async () => {
      await expect(pipeline.executeStage('non-existent', createMockContext())).rejects.toThrow(/not found/i);
    });
  });
});
