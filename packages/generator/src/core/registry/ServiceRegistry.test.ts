...
function createMockService(name: string, dependencies: string[] = [], version = '1.0.0'): IService {
  return {
    name,
    version,
    dependencies,
...
describe('ServiceRegistry', () => {
  ...
  describe('getInitializationOrder', () => {
    ...
    it('should warn about missing dependencies but continue', () => {
      ...
      expect(order).toContain('service-a');
      expect(order).toContain('service-b');
      expect(consoleWarn).toHaveBeenCalledWith(expect.stringContaining('missing dependency'));
  ...
  describe('initializeAll', () => {
    ...
    it('should throw if a service initialization fails', async () => {
      ...
      (service.initialize as ReturnType<typeof vi.fn>).mockRejectedValue(new Error('Init failed'));
  ...
  describe('disposeAll', () => {
    ...
    it('should continue disposing other services if one fails', async () => {
      ...
      (serviceA.dispose as ReturnType<typeof vi.fn>).mockRejectedValue(new Error('Dispose failed'));
