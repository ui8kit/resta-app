...
export async function runGenerateHtml(context: IPipelineContext, service: HtmlService): Promise<HtmlServiceOutput> {
  ...
  const routes = Object.fromEntries(Object.entries(allRoutes).filter(([path]) => !skipRoutes.has(path)));
