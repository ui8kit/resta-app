 ...
 *   6. dist/react/ — generated blocks/layouts/partials exist (if dist present)
 *
 * Usage:
...
async function main(): Promise<void> {
  ...
  // ── 6. dist/react/ generated structure ───────────────────────────────────
  const distReact = join(ROOT, "dist", "react");
  if (existsSync(distReact)) {
    ...
    for (const dir of ["blocks", "layouts", "partials"]) {
      if (!existsSync(join(baseDir, dir))) {
        addIssue(issues, {
          ...
          message: `dist/react/${hasSrc ? "src/" : ""}${dir}/ not found — run: bun run generate`,
        });
      }
    }
    ...
    if (!existsSync(join(distReact, "package.json"))) {
      addIssue(issues, {
        ...
        message: "dist/react/package.json missing — run: bun run finalize",
      });
    }
  }
